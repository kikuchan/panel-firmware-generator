...
function serializeDsiModeFlagsForDTS(flags: FlagsObject<DsiModeFlags>) {
  const keys = Object.keys(flags);
  if (keys.length === 0) return undefined;
  return keys.map((k) => `dsi-${k.toLowerCase().replace(/_/g, '-')};`).join('\n');
}

function serializeDrmFlagsForDTS(
  modeFlags: FlagsObject<DrmModeFlags>,
  busFlags: FlagsObject<DrmBusFlags>,
): string | undefined {
  const result: string[] = [];

  if (modeFlags.PVSYNC || modeFlags.NVSYNC) {
    result.push(`vsync-active = <${modeFlags.PVSYNC ? 1 : 0}>;`);
  }
  if (modeFlags.PHSYNC || modeFlags.NHSYNC) {
    result.push(`hsync-active = <${modeFlags.PHSYNC ? 1 : 0}>;`);
  }

  if (busFlags.DE_LOW || busFlags.DE_HIGH) {
    result.push(`de-active = <${busFlags.DE_HIGH ? 1 : 0}>;`);
  }
  if (busFlags.PIXDATA_DRIVE_POSEDGE || busFlags.PIXDATA_DRIVE_NEGEDGE) {
    result.push(`pixelclk-active = <${busFlags.PIXDATA_DRIVE_POSEDGE ? 1 : 0}>;`);
  }
  if (busFlags.SYNC_DRIVE_POSEDGE || busFlags.SYNC_DRIVE_NEGEDGE) {
    result.push(`syncclk-active = <${busFlags.SYNC_DRIVE_POSEDGE ? 1 : 0}>;`);
  }

  if (modeFlags.INTERLACE) {
    result.push(`interlaced;`);
  }
  if (modeFlags.DBLSCAN) {
    result.push(`doublescan;`);
  }
  if (modeFlags.DBLCLK) {
    result.push(`doubleclk;`);
  }

  return result.length > 0 ? result.join('\n') : undefined;
}

function coalesce(texts: TemplateStringsArray, ...values: unknown[]) {
  if (values.some((x) => x == null || x === false)) return '';
  return texts.flatMap((text, idx) => (idx ? [String(values[idx - 1]), text] : [text])).join('');
}

function indent(n: number, text: string | undefined) {
  if (text === undefined) return undefined;
  return text
    .split('\n')
    .map((x) => (x.trim() ? ' '.repeat(n) + x : ''))
    .join('\n');
}
...
function toDTS(config: ParsedConfig, opts: DtsOptions = {}) {
  ...
  return indent(
    opts.indent ?? 0,
    `
width-mm = <${serialized.width_mm}>;
height-mm = <${serialized.height_mm}>;
...
display-timings {
  ...
  timing${idx} {
    ...
    hfront-porch = <${timing.hfp}>;
    hsync-len = <${timing.hslen}>;
    hback-porch = <${timing.hbp}>;

    vactive = <${timing.vactive}>;
    vfront-porch = <${timing.vfp}>;
    vsync-len = <${timing.vslen}>;
    vback-porch = <${timing.vbp}>;${coalesce`

${indent(4, serializeDrmFlagsForDTS(timing.flags, serialized.bus_flags))}`}
  };`,
      )
      .join('\n')
  }
